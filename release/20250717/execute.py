...
def set_column_widths(worksheet, widths: List[int]):
    """设置工作表的列宽，支持超长列（>26列）和更大宽度"""
    from openpyxl.utils import get_column_letter
    for i, width in enumerate(widths, 1):
        # 适当放大宽度，1字符≈1.2excel宽度
        excel_width = width * 1.2
        worksheet.column_dimensions[get_column_letter(i)].width = excel_width
...
def get_cell_color(header: str, value: str, row_data: dict) -> Optional[str]:
    ...
    if header == '状态':
        ...
        # 如果实际缺勤时间 > 0 且状态是正常，标记为更亮更深的黄色
        minutes = int(value)
        if minutes > 0 and row_data['状态'] == '正常':
            return 'FFFF00'  # 更亮更深的金黄色
...
def save_reports(statistics_data, summary_data, start_date, end_date):
    ...
    if statistics_data:
        ...
        # 写入表头
        # 调整列顺序：将“情况说明”移到“临时卡”后面
        orig_headers = list(df_stats.columns)
        # 新顺序：在“临时卡”后插入“情况说明”
        if '情况说明' in orig_headers and '临时卡' in orig_headers:
            orig_headers.remove('情况说明')
            idx = orig_headers.index('临时卡') + 1
            orig_headers.insert(idx, '情况说明')
        headers = orig_headers
        for col, header in enumerate(headers, 1):
            cell = ws_stats.cell(row=1, column=col)
            apply_cell_format(cell, header, is_header=True, bg_color='CCCCCC', bold=True)
            # 强制加黑色边框
            cell.border = Border(left=Side(style='thin', color='000000'), right=Side(style='thin', color='000000'), top=Side(style='thin', color='000000'), bottom=Side(style='thin', color='000000'))
        
        # 设置列宽 - 考勤统计表
        # 日期(15) 姓名(12) 部门(30) 上班时间(12) 下班时间(12) 工作时长(10) 
        # 当天请假时长(20) 午休时长(10) 实际请假时长(20) 实际缺勤时间[分钟](20) 
        # 应出勤时长(12) 情况说明(50) 状态(20) 临时卡(8) 备注(20)
        set_column_widths(ws_stats, [15, 12, 30, 12, 12, 10, 20, 10, 20, 20, 12, 16, 8, 38, 20])
